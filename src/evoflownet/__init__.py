...
- :mod:`evoflownet.landscapes` -- fitness functions (closed-form or empirical)
- :mod:`evoflownet.env`        -- the state graph variants are built through
- :mod:`evoflownet.algorithms` -- samplers: GFlowNets and classical baselines
- :mod:`evoflownet.metrics`    -- evaluation, including exact-distribution checks
- :mod:`evoflownet.loop`       -- design-build-test-learn campaigns under a budget
"""

from importlib.metadata import PackageNotFoundError, version

try:
    __version__ = version("evoflownet")
